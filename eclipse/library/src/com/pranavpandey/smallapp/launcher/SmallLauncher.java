	...
	public static boolean launchSmallApp(Context context, PackageManager packageManager, 
		...
		try {
			if (SdkInfo.VERSION.API_LEVEL >= 2) {
				SmallApplicationManager.startApplication(context, getSmallAppIntent(context, packageManager, packageName));
	 		} else {
				Intent smallAppConnectionIntent = new Intent();
				smallAppConnectionIntent.setAction(ACTION_START_APPLICATION);
				smallAppConnectionIntent.setComponent(new ComponentName("com.sony.smallapp.managerservice", 
						"com.sony.smallapp.managerservice.SmallAppManagerService"));
				StartSmallAppConnection localStartSmallAppConnection = new StartSmallAppConnection(context, 
						new ComponentName(packageName, getSmallAppClassName(packageManager, packageName)));
