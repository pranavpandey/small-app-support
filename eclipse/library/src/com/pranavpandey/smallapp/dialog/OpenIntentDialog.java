...
public class OpenIntentDialog extends ActionDialog {
     ...
     */
    public OpenIntentDialog(Context context, Intent intent,
                            AlertDialog.Builder dialogBuilder, Type type) {
        ...
        if (launchables.isEmpty()) {
            ...
            if (mNoActivityListener != null) {
                ...
                View mView = LayoutInflater.from(getContext()).inflate(
                        R.layout.sas_dialog_warning_info,
                        new LinearLayout(getContext()), false);
                ((TextView) mView.findViewById(R.id.warning_message)).setText(
                        R.string.sas_error_no_apps);

                if (isExtraInfo()) {
                    ((ColoredTextView) mView.findViewById(R.id.extra_info_desc))
                            .setText(getExtraInfoText());
                ...
                SmallUtils.createDialog(getDialogBuilder().create(),
                        rootView.getWindowToken()).show();
            }
        } else {
            ...
            if (launchables.size() == 1 && mOpenSingleApp) {
                ...
                ActivityAdapter adapter = new ActivityAdapter(getContext(), mPackageManager,
                        getType() == Type.LIST ? R.layout.sas_row_list_action
                                : R.layout.sas_item_grid_action);
     ...
     * to set methods.
     */
    public OpenIntentDialog setNoActivityListener(
            @Nullable OnNoActivityListener noActivityListener) {
     ...
     * to set methods.
     */
    public OpenIntentDialog setActivityOpenListener(
            @Nullable OnActivityOpenListener activityOpenListener) {
